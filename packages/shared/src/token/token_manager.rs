...
impl TokenManager {
    ...
    pub fn new(tokens: Vec<CryptoToken>, network: &NetworkKind) -> Self {
        ...
        let native_token: Arc<CryptoToken> = TokenManager::get_by_address_str_impl(&tokens, native_token_address).unwrap();
        Self {
            tokens,
            native_token,
        }
    ...
    pub fn get_by_address(&self, address: &Address) -> Option<Arc<CryptoToken>> {
        self.tokens.iter().find(|token| token.address() == address).cloned()
    }

    pub fn get_by_address_str(&self, address: &str) -> Option<Arc<CryptoToken>> {
        TokenManager::get_by_address_str_impl(&self.tokens, address)
    }

    pub fn get_by_symbol(&self, symbol: &str) -> Option<Arc<CryptoToken>> {
        self.tokens.iter().find(|token| token.symbol() == symbol).cloned()
    }
}
