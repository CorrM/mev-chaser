    ...
    M::Error: 'static,
{
    ...
    #[inline]
    fn execute(&self, mut action: SubmitTxInfo) -> Result<Option<Notification>> {
        let gas_usage: U256 =
            block_on(self.client.estimate_gas(&action.tx, None)).context("Error estimating gas usage: {}")?;
        ...
        if let Some(gas_bid_info) = action.gas_bid_info {
            ...
            bid_gas_price = block_on(self.client.get_gas_price()).context("Error getting gas price: {}")?;
        }

        action.tx.set_gas_price(bid_gas_price);
        let p_tx: PendingTransaction<<M as Middleware>::Provider> =
            block_on(self.client.send_transaction(action.tx.clone(), None))?;
